...
class AITradingStrategy {
    constructor(tokenAddress, connection, initialParameters, useAi = false, apiKey) {
        ...
        this.solPrice = 150; // Default fallback price in USD
    ...
    async update() {
        const now = Date.now();
        console.log(chalk_1.default.magenta('==== AI STRATEGY UPDATE CALLED ===='));
        console.log(chalk_1.default.magenta(`Token Address: ${this.tokenAddress}`));
        console.log(chalk_1.default.magenta(`Last Updated: ${new Date(this.lastUpdated).toISOString()}`));
        console.log(chalk_1.default.magenta(`Update Interval: ${this.updateInterval / 1000} seconds`));
        console.log(chalk_1.default.magenta(`Time Since Last Update: ${(now - this.lastUpdated) / 1000} seconds`));
        ...
        if (now - this.lastUpdated > this.updateInterval) {
            try {
                ...
                console.log(chalk_1.default.magenta(`Token Metrics: Liquidity: $${metrics.liquidity || 'Unknown'}`));
                console.log(chalk_1.default.magenta(`Token Metrics: Data Quality: ${metrics.dataQuality}`));
                console.log(chalk_1.default.magenta(`Token Metrics: Is Low Liquidity: ${metrics.isLowLiquidity}`));
                // Optimize parameters if AI is enabled
                if (this.useAi && this.openai) {
                    console.log(chalk_1.default.magenta('Calling AI for recommendations...'));
                    ...
                    if (recommendation) {
                        ...
                        if (recommendation.confidence >= 70) {
                            ...
                            if (recommendation.parameters.maxAmount > previousMaxAmount) {
                                console.log(chalk_1.default.yellow('⚠️ BALANCE ALERT: AI is recommending a higher max trade amount!'));
                                console.log(chalk_1.default.yellow(`Previous max: ${previousMaxAmount} SOL → New max: ${recommendation.parameters.maxAmount} SOL`));
                                console.log(chalk_1.default.yellow(`Make sure your wallets have enough SOL balance (at least ${(recommendation.parameters.maxAmount * 1.5).toFixed(2)} SOL recommended)`));
                            }
        ...
        if (this.metrics && this.metrics.liquidity !== undefined) {
            ...
            if (this.metrics.liquidity < 10) {
                ...
                console.log(chalk_1.default.red('ENFORCING LIMITS: Extremely low liquidity (<$10) - Max 0.05 SOL'));
            }
            else if (this.metrics.liquidity < 100) {
                maxAllowedAmount = 0.1;
                console.log(chalk_1.default.red('ENFORCING LIMITS: Very low liquidity (<$100) - Max 0.1 SOL'));
            }
            else if (this.metrics.liquidity < 500) {
                maxAllowedAmount = 0.5;
                console.log(chalk_1.default.red('ENFORCING LIMITS: Low liquidity (<$500) - Max 0.5 SOL'));
            }
            else if (this.metrics.liquidity < 1000) {
                maxAllowedAmount = 1;
                console.log(chalk_1.default.red('ENFORCING LIMITS: Moderately low liquidity (<$1000) - Max 1 SOL'));
            }
            else if (this.metrics.liquidity < 5000) {
                maxAllowedAmount = 5;
                console.log(chalk_1.default.red('ENFORCING LIMITS: Moderate liquidity (<$5000) - Max 5 SOL'));
            }
            else if (this.metrics.liquidity < 20000) {
                maxAllowedAmount = 10;
                console.log(chalk_1.default.red('ENFORCING LIMITS: Good liquidity (<$20000) - Max 10 SOL'));
            }
            else if (this.metrics.liquidity < 50000) {
                maxAllowedAmount = 20;
                console.log(chalk_1.default.red('ENFORCING LIMITS: High liquidity (<$50000) - Max 20 SOL'));
            }
            else if (this.metrics.liquidity < 200000) {
                maxAllowedAmount = 30;
                console.log(chalk_1.default.red('ENFORCING LIMITS: Very high liquidity (<$200000) - Max 30 SOL'));
            }
            else {
                maxAllowedAmount = 50;
                console.log(chalk_1.default.red('ENFORCING LIMITS: Extremely high liquidity (>$200000) - Max 50 SOL'));
            }
            ...
            if (this.currentParameters.maxAmount > maxAllowedAmount) {
                ...
                console.log(chalk_1.default.red(`DIRECT OVERRIDE: Max amount set to ${maxAllowedAmount} SOL based on liquidity tier`));
            }
                ...
                console.log(chalk_1.default.yellow('⚠️ BALANCE ALERT: Liquidity has increased, allowing for higher trade amounts'));
                console.log(chalk_1.default.yellow(`Previous max: ${previousMaxAmount} SOL → New max: ${this.currentParameters.maxAmount} SOL`));
                console.log(chalk_1.default.yellow(`Make sure your wallets have enough SOL balance (at least ${(this.currentParameters.maxAmount * 1.5).toFixed(2)} SOL recommended)`));
            }
            ...
            if (this.currentParameters.minAmount > minAmount * 2 || this.currentParameters.minAmount < minAmount / 2) {
                ...
                console.log(chalk_1.default.red(`DIRECT OVERRIDE: Min amount set to ${minAmount} SOL (10% of max)`));
            }
        }
        // Final safety check to ensure parameters are reasonable
        if (this.currentParameters.maxAmount > 50) {
            console.log(chalk_1.default.red(`SAFETY CHECK: Reducing max amount from ${this.currentParameters.maxAmount} to 50 SOL (global maximum)`));
            this.currentParameters.maxAmount = 50;
        }
        if (this.currentParameters.minAmount < 0.01) {
            console.log(chalk_1.default.red(`SAFETY CHECK: Increasing min amount from ${this.currentParameters.minAmount} to 0.01 SOL (global minimum)`));
            this.currentParameters.minAmount = 0.01;
        }
        console.log(chalk_1.default.magenta('==== FINAL PARAMETERS FROM AI STRATEGY ===='));
        console.log(chalk_1.default.magenta(`Max Amount: ${this.currentParameters.maxAmount} SOL`));
        console.log(chalk_1.default.magenta(`Min Amount: ${this.currentParameters.minAmount} SOL`));
        console.log(chalk_1.default.magenta(`Time Between: ${this.currentParameters.timeBetween}ms`));
        console.log(chalk_1.default.magenta(`Number of Buys: ${this.currentParameters.numBuys}`));
    ...
    async fetchTokenMetrics() {
        ...
        try {
            ...
            if (response.data && response.data.pairs && response.data.pairs.length > 0) {
                ...
                console.log(chalk_1.default.blue(`Raw liquidity from DexScreener: $${metrics.liquidity}`));
                // Handle case where liquidity shows as 0 but data exists (workaround for bug)
                if (metrics.liquidity === 0 && metrics.price > 0) {
                    console.log(chalk_1.default.yellow('DexScreener reported 0 liquidity but token has price data. Setting to conservative liquidity estimate.'));
                ...
                if (metrics.liquidity === 0 || metrics.liquidity === undefined) {
                    ...
                    console.log(chalk_1.default.red(`WARNING: No liquidity data found for ${this.tokenAddress}. Setting to minimum value (1 USD).`));
                }
            ...
            if (metrics.liquidity === 0 || metrics.liquidity === undefined) {
                metrics.liquidity = 1; // 1 USD liquidity for tokens with no data
                console.log(chalk_1.default.red(`WARNING: Error fetching metrics for ${this.tokenAddress}. Setting liquidity to minimum value (1 USD).`));
            }
            // If we still have no data and previous metrics exist, use them
            if ((metrics.dataQuality === 'none' || metrics.dataQuality === 'low') && this.metrics) {
    ...
    async fetchSolPrice() {
        try {
            ...
            if (response.data && response.data.solana && response.data.solana.usd) {
                ...
                console.log(chalk_1.default.blue(`Current SOL price: $${this.solPrice}`));
            ...
            try {
                const fallbackResponse = await axios_1.default.get('https://price.jup.ag/v4/price?ids=SOL', {
                    timeout: 5000
                });
                ...
                console.log(chalk_1.default.yellow(`Could not fetch SOL price, using default: $${this.solPrice}`));
            }
        }
     ...
     */
    async fetchFallbackMetrics(metrics) {
        try {
            ...
            try {
                ...
                const jupResp = await axios_1.default.get(jupiterUrl, { timeout: 5000 });
                ...
                console.log(chalk_1.default.yellow('Jupiter price API check failed'));
            }
            ...
            console.error(chalk_1.default.red(`Error in fallback metrics: ${error.message}`));
            metrics.dataQuality = 'none';
            metrics.isLowLiquidity = true;
        }
    }
    ...
    async getAIRecommendation() {
        ...
        try {
            ...
            console.log(chalk_1.default.cyan('=== DEBUG: AI RECOMMENDATION PROCESS STARTING ==='));
            console.log(chalk_1.default.cyan(`Token Address: ${tokenAddress}`));
            console.log(chalk_1.default.cyan(`Liquidity: $${currentMetrics.liquidity || 'Unknown'}`));
            console.log(chalk_1.default.cyan(`SOL Price: $${this.solPrice}`));
...
Note that I have ALREADY specified the maxAmount as ${exactMaxAmount} SOL in the template above, which is the correct value for this token's liquidity. DO NOT CHANGE THIS VALUE.`;
            ...
            console.log(chalk_1.default.cyan('Sending request to OpenAI...'));
...
Remember that incorrect trade sizes relative to liquidity can cause significant issues including slippage or failing to generate proper momentum.`
            ...
            console.log(chalk_1.default.cyan('Received response from OpenAI'));
            ...
            try {
                console.log(chalk_1.default.cyan('Parsing AI response content:'));
                console.log(content);
                const recommendation = JSON.parse(content);
                console.log(chalk_1.default.cyan('AI recommended parameters:'));
                console.log(chalk_1.default.cyan(`Max Amount: ${recommendation.parameters.maxAmount} SOL`));
                console.log(chalk_1.default.cyan(`Min Amount: ${recommendation.parameters.minAmount} SOL`));
                console.log(chalk_1.default.cyan(`Time Between: ${recommendation.parameters.timeBetween}ms`));
                console.log(chalk_1.default.cyan(`Number of Buys: ${recommendation.parameters.numBuys}`));
                // Verify the AI followed instructions for max amount
                if (recommendation.parameters.maxAmount !== exactMaxAmount) {
                    console.log(chalk_1.default.red(`AI didn't follow instructions for maxAmount. Fixing value from ${recommendation.parameters.maxAmount} to ${exactMaxAmount} SOL`));
                    recommendation.parameters.maxAmount = exactMaxAmount;
                }
                else {
                    console.log(chalk_1.default.green(`AI correctly used maxAmount: ${exactMaxAmount} SOL`));
                }
                ...
                if (currentMetrics.liquidity !== undefined) {
                    ...
                    if (recommendation.parameters.minAmount < minRecommendedAmount || recommendation.parameters.minAmount > maxRecommendedMin) {
                        ...
                        console.log(chalk_1.default.yellow(`ENFORCED: Adjusted min amount from ${originalMin} to ${recommendation.parameters.minAmount} SOL (5-20% of max)`));
                    }
                    ...
                    if (recommendation.parameters.timeBetween < minTimeBetween) {
                        ...
                        console.log(chalk_1.default.yellow(`ENFORCED: Increased time between trades from ${originalTime}ms to ${minTimeBetween}ms`));
                    }
                    ...
                    if (recommendation.parameters.numBuys > maxBuys) {
                        ...
                        console.log(chalk_1.default.yellow(`ENFORCED: Reduced number of buys from ${originalBuys} to ${maxBuys}`));
                    }
                }
                console.log(chalk_1.default.cyan('Final AI recommendation parameters after enforcement:'));
                console.log(chalk_1.default.cyan(`Max Amount: ${recommendation.parameters.maxAmount} SOL`));
                console.log(chalk_1.default.cyan(`Min Amount: ${recommendation.parameters.minAmount} SOL`));
                console.log(chalk_1.default.cyan(`Time Between: ${recommendation.parameters.timeBetween}ms`));
                console.log(chalk_1.default.cyan(`Number of Buys: ${recommendation.parameters.numBuys}`));
                console.log(chalk_1.default.cyan('=== DEBUG: AI RECOMMENDATION PROCESS COMPLETE ==='));
            ...
            if (error.response) {
                console.error(chalk_1.default.red(`OpenAI API Error: ${JSON.stringify(error.response.data || {})}`));
            }
            return null;
        }
    }
