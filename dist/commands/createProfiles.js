...
exports.createProfilesCommand = void 0;
 ...
 */
async function generateAIUsername(openaiKey) {
    try {
        if (!openaiKey) {
        ...
        console.log(chalk_1.default.yellow("Error generating AI username, falling back to standard random username"));
 ...
 */
async function generateAIBio(openaiKey) {
    try {
        if (!openaiKey) {
            throw new Error("No OpenAI API key provided");
        }
        ...
        console.log(chalk_1.default.yellow("Error generating AI bio, using default bio"));
        // Fallback to a default bio
        return "Crypto enthusiast. Diamond hands. Always looking for the next gem.";
    }
}
async function createProfilesCommand(options) {
    try {
        ...
        if (!isConnected) {
            console.log(chalk_1.default.red('Cannot proceed without API connectivity.'));
            return;
        }
        ...
        if (useProxy && proxiesAvailable) {
            console.log(chalk_1.default.green('✓ Using residential proxies for profile creation'));
            ...
            if (testResult.success) {
                ...
                console.log(chalk_1.default.yellow(`Proxy test failed: ${testResult.message}. Will retry during profile creation.`));
            }
        }
        ...
        if (!useAi && username === undefined) {
            ...
            if (!randomUsername) {
                const customUsernameAnswer = await inquirer_1.default.prompt([
                    {
                        type: 'input',
                        name: 'username',
        ...
        console.error(chalk_1.default.red(`Error creating profiles: ${error.message}`));
    }
}
exports.createProfilesCommand = createProfilesCommand;
...
async function createProfiles(wallets, options) {
    const spinner = (0, ora_1.default)('Creating PumpFun profiles...').start();
    let successCount = 0;
    let failureCount = 0;
    const MAX_RETRIES = 3;
    ...
    for (let i = 0; i < wallets.length; i++) {
        ...
        try {
            ...
            const authResult = await (0, PumpFunWrapper_1.enhancedAuthenticate)(wallet, useProxy ? { useProxy, sessionId } : undefined);
            if (!authResult) {
                failureCount++;
            ...
            if (options.withImage) {
                ...
                try {
                    profileImageUrl = await (0, imageUpload_1.uploadImage)(authResult.authToken, useProxy, sessionId);
                    if (!profileImageUrl) {
                        console.log(chalk_1.default.yellow(`No image found or upload failed for wallet ${wallet.publicKey.substring(0, 8)}. Continuing without image.`));
                    }
                }
                catch (imageError) {
                    console.log(chalk_1.default.yellow(`Error uploading image: ${imageError.message}. Continuing without image.`));
                }
            }
            // Generate username
            let username;
            if (options.useAi && options.openaiKey) {
                ...
                if (options.openaiKey) {
                    ...
                    console.log(chalk_1.default.cyan(`Generated AI username: "${username}"`));
                }
                else {
                    // Fallback to random username if no API key
                    username = generateRandomUsername();
                    console.log(chalk_1.default.yellow(`No OpenAI API key provided, using random username: "${username}"`));
                }
            }
            ...
            if (options.useAi && options.openaiKey) {
                ...
                if (options.openaiKey) {
                    ...
                    console.log(chalk_1.default.cyan(`Generated AI bio: "${bio}"`));
                }
                else {
                    // Fallback to default bio if no API key
                    bio = "Crypto enthusiast. Diamond hands. Always looking for the next gem.";
                    console.log(chalk_1.default.yellow(`No OpenAI API key provided, using default bio`));
                }
            }
            else {
                bio = options.bio;
            ...
            for (let attempt = 0; attempt < MAX_RETRIES && !profileCreated; attempt++) {
                try {
                    if (attempt > 0) {
                        ...
                        await (0, transaction_1.sleep)(RETRY_DELAY * Math.pow(2, attempt - 1));
                    }
                    ...
                    const proxyError = error.message && (error.message.includes('proxy') ||
                    ...
                    const networkError = error.message && (error.message.includes('network') ||
                    ...
                    if (socketHangup || timeout || connectionReset || networkError) {
                        ...
                        await (0, transaction_1.sleep)(3000);
        ...
        if (i < wallets.length - 1) {
            ...
            console.log(chalk_1.default.gray(`Waiting ${Math.round(delayAmount / 1000)} seconds before next wallet...`));
            await (0, transaction_1.sleep)(delayAmount);
        }
    }
    spinner.succeed('Profile creation complete');
    // Display summary
    console.log('\n' + chalk_1.default.cyan('====== PROFILE CREATION SUMMARY ======'));
    console.log(chalk_1.default.green(`Total wallets processed: ${wallets.length}`));
    console.log(chalk_1.default.green(`Successful profiles: ${successCount}`));
    console.log(chalk_1.default.green(`Failed profiles: ${failureCount}`));
    console.log(chalk_1.default.cyan('===================================='));
    if (failureCount > 0) {
        console.log(chalk_1.default.yellow('\nSome profiles failed to create. Common issues:'));
        console.log(chalk_1.default.yellow('1. Username already taken - try a different username'));
        console.log(chalk_1.default.yellow('2. Too many requests - try again in a few minutes'));
        console.log(chalk_1.default.yellow('3. Connection issues - check your internet connection'));
        console.log(chalk_1.default.yellow('4. API changes - pump.fun may have updated their API'));
    }
}
/**
 ...
 */
async function verifyProfileCreation(authToken, publicKey, useProxy = false, sessionId) {
    try {
...
async function checkApiConnectivity() {
    try {
        console.log(chalk_1.default.cyan('Checking connection to pump.fun API...'));
        ...
        if (proxyManager.isEnabled()) {
            ...
            const testResult = await proxyManager.testProxy();
            if (testResult.success) {
                console.log(chalk_1.default.green(`Using proxy for API connectivity test: ${testResult.ip}`));
                // Apply proxy configuration
                const proxyConfig = proxyManager.getAxiosConfig();
                clientConfig = { ...clientConfig, ...proxyConfig };
            }
            else {
                console.log(chalk_1.default.yellow(`Proxy test failed, using direct connection for API check: ${testResult.message}`));
            }
        }
        const client = axios_1.default.create(clientConfig);
        ...
        console.log(chalk_1.default.red('Error checking API connectivity.'));
        return false;
    }
}
