...
async function createToken(options) {
    try {
        ...
        if (wallets.length < totalWalletsNeeded) {
            console.log(chalk_1.default.yellow(`Warning: Not enough wallets for ${options.initialBuys} initial buys. Will use ${wallets.length - 1} wallets instead.`));
            options.initialBuys = Math.max(0, wallets.length - 1);
        }
        // Prepare signer keypairs
        console.log(chalk_1.default.cyan('Preparing wallet keypairs...'));
        ...
        if (options.initialBuys > 0) {
            ...
            for (const buyerWallet of buyerWallets) {
                ...
                console.log(chalk_1.default.cyan(`Added buyer wallet: ${buyerKeypair.publicKey.toString()}`));
            }
        }
        // Generate a random keypair for the mint
        console.log(chalk_1.default.cyan('Generating mint keypair...'));
        const mintKeypair = web3_js_1.Keypair.generate(); // Generates a random keypair for token
        const mintPublicKey = mintKeypair.publicKey.toBase58();
        console.log(chalk_1.default.cyan(`Generated mint address: ${mintPublicKey}`));
        // Check logo file
        if (!fs_1.default.existsSync(options.logoPath)) {
            throw new Error(`Logo file not found: ${options.logoPath}`);
        }
        // Read logo file
        console.log(chalk_1.default.blue(`Reading logo file...`));
        const fileData = fs_1.default.readFileSync(options.logoPath);
        const filename = path_1.default.basename(options.logoPath);
        // Upload to IPFS
        console.log(chalk_1.default.blue(`Uploading metadata to IPFS...`));
        const formData = new form_data_1.default();
        formData.append("file", fileData, {
            filename: filename,
            contentType: `image/${path_1.default.extname(filename).substring(1)}`
        });
        ...
        console.log(chalk_1.default.green(`Metadata uploaded successfully: ${metadataUri}`));
        ...
        console.log(chalk_1.default.cyan(`Requesting trade transactions for ${bundledTxArgs.length} operations...`));
        try {
            const response = await axios_1.default.post("https://pumpportal.fun/api/trade-local", bundledTxArgs, {
                headers: {
                    "Content-Type": "application/json"
                }
            });
            if (response.status === 200) {
                console.log(chalk_1.default.green('Successfully generated transactions'));
                const transactions = response.data;
                let encodedSignedTransactions = [];
                let signatures = [];
                console.log(chalk_1.default.cyan(`Received ${transactions.length} transactions to sign`));
                // Sign transactions
                for (let i = 0; i < bundledTxArgs.length; i++) {
                    console.log(chalk_1.default.cyan(`Signing transaction ${i + 1}/${bundledTxArgs.length}...`));
                    try {
                        const tx = web3_js_1.VersionedTransaction.deserialize(new Uint8Array(bs58_1.default.decode(transactions[i])));
                        if (bundledTxArgs[i].action === "create") {
                            // Creation transaction needs to be signed by mint and creator keypairs
                            console.log(chalk_1.default.cyan(`Signing creation tx with mint and creator keypairs`));
                            tx.sign([mintKeypair, signerKeyPairs[0]]);
                        }
                        else {
                            // Buy transactions signed by corresponding wallet
                            console.log(chalk_1.default.cyan(`Signing buy tx with buyer keypair ${i}`));
                            tx.sign([signerKeyPairs[i]]);
                        }
                        encodedSignedTransactions.push(bs58_1.default.encode(tx.serialize()));
                        signatures.push(bs58_1.default.encode(tx.signatures[0]));
                    }
                    catch (signError) {
                        console.error(chalk_1.default.red(`Error signing transaction ${i}: ${signError.message}`));
                        throw new Error(`Transaction signing failed: ${signError.message}`);
                    }
                }
                // Submit to Jito bundle
                console.log(chalk_1.default.cyan(`Submitting ${encodedSignedTransactions.length} transactions to Jito...`));
                try {
                    ...
                    if (jitoResponse.data && jitoResponse.data.result) {
                        console.log(chalk_1.default.green(`Bundle submitted to Jito: ${jitoResponse.data.result}`));
                    }
                    else if (jitoResponse.data && jitoResponse.data.error) {
                        console.error(chalk_1.default.red(`Jito error: ${JSON.stringify(jitoResponse.data.error)}`));
                        console.log(chalk_1.default.yellow('Transactions may still be processed individually...'));
                    }
                }
                catch (jitoError) {
                    console.error(chalk_1.default.red(`Error submitting to Jito: ${jitoError.message}`));
                    console.log(chalk_1.default.yellow('Transactions may still be processable individually...'));
                }
                // Print transaction links
                console.log(chalk_1.default.cyan('\n===== Transaction Summary ====='));
                for (let i = 0; i < signatures.length; i++) {
                    console.log(chalk_1.default.cyan(`Transaction ${i}: https://solscan.io/tx/${signatures[i]}`));
                }
                // Print token link
                console.log(chalk_1.default.cyan(`\nView token on Solscan: https://solscan.io/token/${mintPublicKey}`));
                console.log(chalk_1.default.cyan(`View token on Birdeye: https://birdeye.so/token/${mintPublicKey}?chain=solana`));
                return {
                    success: true,
            ...
            console.error(chalk_1.default.red(`Error requesting or processing transactions: ${error.message}`));
            return {
                success: false,
                error: error.message
            };
