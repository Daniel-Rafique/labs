...
exports.enhancedBulkLikeComments = exports.fetchReplies = exports.enhancedLikeComment = exports.enhancedAuthenticate = exports.enhancedCheckCommentsEnabled = exports.enhancedPostComment = void 0;
 ...
 */
function convertWalletFormat(wallet) {
    return {
...
async function enhancedPostComment(wallet, tokenMint, comment, proxy, withImage = false) {
    try {
        ...
        if (withImage) {
            ...
            try {
                ...
                if (uploadResult) {
                    ...
                    console.log(chalk_1.default.green(`Successfully uploaded image: ${imageUrl}`));
                }
                else {
                    console.log(chalk_1.default.yellow('Image upload failed or no image found, proceeding without image'));
                }
            }
            catch (imageError) {
                console.log(chalk_1.default.yellow(`Error uploading image: ${imageError.message}. Proceeding without image.`));
            }
        }
        ...
        console.error(chalk_1.default.red(`Enhanced posting failed: ${error.message}`));
        return false;
    }
}
exports.enhancedPostComment = enhancedPostComment;
/**
...
exports.enhancedCheckCommentsEnabled = enhancedCheckCommentsEnabled;
/**
...
async function directPumpSignIn(wallet, proxy) {
    ...
    try {
        ...
        console.log(chalk_1.default.gray(`Created auth payload with timestamp: ${authPayload.timestamp}`));
        // Try to sign in
        console.log(chalk_1.default.gray(`Attempting to authenticate with ${PUMP_FUN_BASE_URL}/auth/login`));
        ...
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                lastError = error;
                console.log(chalk_1.default.yellow(`Authentication attempt ${attempt + 1} failed: ${error.message}`));
                // Check for rate limiting or temporary errors
                if (attempt < MAX_RETRIES - 1) {
                    console.log(chalk_1.default.yellow(`Retrying in ${RETRY_DELAY / 1000} seconds...`));
        ...
        console.log(chalk_1.default.green(`Successfully authenticated wallet ${wallet.publicKey.substring(0, 8)}`));
        // Now get the AWS token
        console.log(chalk_1.default.gray(`Fetching AWS token...`));
        let awsToken = "";
        for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
            try {
                ...
                console.log(chalk_1.default.yellow(`AWS token fetch attempt ${attempt + 1} failed: ${error.message}`));
                if (attempt < MAX_RETRIES - 1) {
                    console.log(chalk_1.default.yellow(`Retrying in ${RETRY_DELAY / 1000} seconds...`));
                    await (0, transaction_1.sleep)(RETRY_DELAY);
                }
        ...
        if (!awsToken) {
            console.log(chalk_1.default.yellow("Could not fetch AWS token, but continuing with auth_token only"));
        }
        else {
            console.log(chalk_1.default.green(`Successfully obtained AWS token`));
        }
...
async function fetchReplies(tokenMint, proxy, authResult) {
    ...
    try {
        ...
        console.error(chalk_1.default.red(`Error fetching replies: ${error.message}`));
        return [];
    }
}
