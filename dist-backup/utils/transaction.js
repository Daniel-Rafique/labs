 ...
 */
function addPriorityFee(transaction, priorityFee = 100000) {
 ...
 */
async function sendTransactionWithRetry(connection, transaction, signers, options = {}, maxRetries = 5) {
    let lastError = null;
 ...
 */
async function sendTransactionWithReliableConfirmation(connection, transaction, signers, options = {}) {
...
exports.sendTransactionWithReliableConfirmation = sendTransactionWithReliableConfirmation;
/**
 * Transfer SOL from one wallet to another
 */
async function transferSol(connection, fromWallet, toWallet, amount) {
    ...
    return await sendTransactionWithRetry(connection, transaction, [fromWallet]);
}
...
async function transferSplToken(connection, fromWallet, toWallet, tokenMint, amount) {
    ...
    const transferInstruction = (0, token_compat_1.createTransferInstruction)(fromTokenAccount.address, toTokenAccount.address, fromWallet.publicKey, amount);
 ...
 */
async function getAccountTokens(connection, ownerAddress) {
    const tokens = [];
 ...
 */
async function sendBundleFromMultipleWallets(connection, sourceWallets, destinationWallet, amounts) {
    ...
    const tipAmount = jito_1.JITO_MIN_TIP_LAMPORTS;
    ...
    for (let i = 0; i < sourceWallets.length; i++) {
        ...
        const tipAccount = new web3_js_1.PublicKey(jito_1.JITO_TIP_ACCOUNTS[Math.floor(Math.random() * jito_1.JITO_TIP_ACCOUNTS.length)]);
    ...
    const serializedTransactions = signedTransactions.map(tx => tx.serialize().toString('base64'));
    // Step 4: Create bundle request
    const bundleRequest = {
        jsonrpc: "2.0",
        id: (0, uuid_1.v4)(), // Use a unique ID for each request
        method: "sendBundle",
        params: [
    ...
    const jitoEndpoints = jito_1.JITO_BUNDLE_ENDPOINTS;
    let bundleId = '';
    let lastError = null;
    for (const endpoint of jitoEndpoints) {
        try {
            console.log(`Trying Jito endpoint: ${endpoint}`);
            const response = await axios_1.default.post(endpoint, bundleRequest, {
    ...
    while (Date.now() - startTime < timeout) {
        try {
            ...
            const statusResponse = await axios_1.default.post(jitoEndpoints[0], statusRequest, {
                headers: { 'Content-Type': 'application/json' },
                timeout: 10000
            });
...
exports.sendBundleFromMultipleWallets = sendBundleFromMultipleWallets;
/**
 ...
 * @returns Transaction signature
 */
async function sendBundleToMultipleWallets(connection, sourceWallet, destinationWallets, amounts) {
    ...
    transaction.add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({
        microLamports: jito_1.JITO_PRIORITY_FEE_MICROLAMPORTS
    }));
    // Set blockhash and fee payer
    transaction.recentBlockhash = blockhash;
    transaction.feePayer = sourceWallet.publicKey;
...
exports.sendBundleToMultipleWallets = sendBundleToMultipleWallets;
/**
 ...
 */
async function bundleTokenTransfersFromSubwallets(connection, sourceWallets, destinationWallet, tokenMints, amounts) {
        ...
        try {
            ...
            for (let i = 0; i < batchWallets.length; i++) {
                ...
                try {
                    ...
                    const destTokenAccount = await (0, token_compat_1.getOrCreateAssociatedTokenAccount)(connection, sourceWallet, tokenMint, destinationWallet);
                    // Create a transaction with the necessary instructions
                    const transaction = new web3_js_1.Transaction();
                    ...
                    transaction.add((0, token_compat_1.createTransferInstruction)(sourceTokenAccount.address, destTokenAccount.address, sourceWallet.publicKey, amount));
                    // Set blockhash and fee payer
                    transaction.recentBlockhash = blockhash;
                    transaction.feePayer = sourceWallet.publicKey;
                    // Sign transaction
    ...
    for (let i = 0; i < successfulTransfers.length; i++) {
        ...
        try {
            ...
            const closeTx = new web3_js_1.Transaction();
            // First check which TOKEN program ID to use (TOKEN vs TOKEN_2022)
            let programId = token_compat_1.TOKEN_PROGRAM_ID;
            try {
                const accountInfo = await connection.getAccountInfo(tokenAccount);
                if (accountInfo && accountInfo.owner.equals(token_compat_1.TOKEN_2022_PROGRAM_ID)) {
                    programId = token_compat_1.TOKEN_2022_PROGRAM_ID;
                }
            }
            catch (error) {
            ...
            closeTx.add((0, token_compat_1.createCloseAccountInstruction)(tokenAccount, keypair.publicKey, // Send rent back to owner
            keypair.publicKey, // Authority
            programId));
            // Add priority fee for faster processing
            closeTx.add(web3_js_1.ComputeBudgetProgram.setComputeUnitPrice({
...
exports.bundleTokenTransfersFromSubwallets = bundleTokenTransfersFromSubwallets;
/**
 ...
 * @returns Transaction signature
 */
async function sendTransactionViaJito(connection, transaction, signers, options = {}) {
    ...
    const hasPriorityFee = transaction.instructions.some(instr => instr.programId.equals(web3_js_1.ComputeBudgetProgram.programId));
    if (!hasPriorityFee) {
        // Use provided priorityFee or default from constants
        const priorityFee = options.priorityFee || jito_1.JITO_PRIORITY_FEE_MICROLAMPORTS;
    ...
    const tipAmount = options.tipAmount === undefined ? jito_1.JITO_MIN_TIP_LAMPORTS : options.tipAmount;
    if (tipAmount > 0) {
        // Select a random tip account
        const tipAccount = new web3_js_1.PublicKey(jito_1.JITO_TIP_ACCOUNTS[Math.floor(Math.random() * jito_1.JITO_TIP_ACCOUNTS.length)]);
        // Add tip instruction after priority fee but before other instructions
        transaction.instructions.splice(1, 0, web3_js_1.SystemProgram.transfer({
    ...
    const jitoEndpoints = jito_1.JITO_TRANSACTION_ENDPOINTS;
    let signature = '';
    let lastError = null;
    for (const endpoint of jitoEndpoints) {
        try {
            console.log(`Trying Jito endpoint: ${endpoint}`);
            const response = await axios_1.default.post(endpoint, transactionRequest, {
                headers: { 'Content-Type': 'application/json' },
                timeout: 30000
            });
            if (response.data.error) {
    ...
    try {
        ...
        signature = await (0, web3_js_1.sendAndConfirmTransaction)(connection, transaction, signers, {
            commitment: 'confirmed',
            skipPreflight: false
        });
...
exports.sendTransactionViaJito = sendTransactionViaJito;
