...
class SolSplWrapper {
    constructor(connection) {
        ...
        this.solSpl.logger = logger_1.default;
    }
    async executeSell(keypair, tokenMint) {
        try {
            // Set the token mint address in process.env
            if (tokenMint) {
                process.env.CONTRACT_ADDRESS = tokenMint;
                logger_1.default.info(`Setting CONTRACT_ADDRESS to ${tokenMint}`);
            }
            else {
                logger_1.default.error('No token mint address provided for sell operation');
                return { success: false, error: 'No token mint address' };
            }
            // Check if PUMPFUN_API_KEY is set
            if (!process.env.PUMPFUN_API_KEY) {
                logger_1.default.warn('No PUMPFUN_API_KEY set in environment. The sell operation might fail.');
                logger_1.default.info('Consider adding your PumpFun API key to the .env file');
            }
            ...
            logger_1.default.error(`Error in executeSell: ${error.message}`);
            return { success: false, error: error.message };
        }
    }
}
async function dustCommand(options) {
    try {
        ...
        if (isNaN(keepAmount) || keepAmount < 0) {
            console.error(chalk_1.default.red('Invalid keep amount. Please provide a valid non-negative number.'));
            return;
        }
        // Determine whether to scan only (no transfers)
        const scanOnly = options.scanOnly === true;
        if (scanOnly) {
            console.log(chalk_1.default.cyan('Running in scan-only mode - no transfers will be made'));
        }
        ...
        if (!scanOnly) {
            const sellTokensAnswer = await inquirer_1.default.prompt([
                {
                    type: 'confirm',
                    name: 'sellTokens',
        ...
        if (options.destination) {
            try {
                destinationPublicKey = new web3_js_1.PublicKey(options.destination);
            }
            catch (error) {
                console.error(chalk_1.default.red('Invalid destination wallet address.'));
                return;
            }
        }
        else {
            ...
            destinationPublicKey = new web3_js_1.PublicKey(selectedWallet);
        }
        console.log(chalk_1.default.cyan(`Destination wallet: ${destinationPublicKey.toString()}`));
        // Set up connection
        ...
        for (let i = 0; i < wallets.length; i++) {
            ...
            try {
                ...
                const keypair = (0, wallet_1.walletDataToKeypair)(wallet);
                ...
                const tokens = await (0, transaction_1.getAccountTokens)(connection, new web3_js_1.PublicKey(wallet.publicKey));
                // Group tokens by mint
                for (const token of tokens) {
                    if (token.amount > 0) {
                        if (!tokensToProcess.has(token.mint)) {
                            tokensToProcess.set(token.mint, {
                                mint: new web3_js_1.PublicKey(token.mint),
                                wallets: []
                            });
                        }
        ...
        console.log(chalk_1.default.cyan(`Found ${tokensToProcess.size} unique token types across all wallets\n`));
        ...
        console.log(chalk_1.default.bold('Top Tokens by Wallet Count:'));
        tokenSummary
            .sort((a, b) => b.walletCount - a.walletCount)
            .slice(0, 5)
            .forEach((token, idx) => {
            console.log(chalk_1.default.cyan(`${idx + 1}. ${token.mint.substring(0, 10)}... - Present in ${token.walletCount} wallets with ${token.totalAmount.toFixed(2)} tokens total`));
        });
        console.log(chalk_1.default.cyan('\n=== SOL Balance Summary ==='));
        let totalSOL = 0;
        ...
        console.log(chalk_1.default.cyan(`Total SOL across all wallets: ${totalSOL.toFixed(6)} SOL`));
        console.log(chalk_1.default.cyan(`Wallets with SOL balance: ${walletsWithSOL}`));
        console.log(chalk_1.default.cyan(`Estimated collectable SOL: ${totalSolCollected.toFixed(6)} SOL`));
        ...
        const { proceedWithTransfers } = await inquirer_1.default.prompt([
            {
                type: 'confirm',
        ...
        if (!proceedWithTransfers) {
            console.log(chalk_1.default.yellow('Transfers canceled by user.'));
            return;
        }
        ...
        if (tokensToProcess.size > 0) {
            ...
            for (const [mintAddress, tokenData] of tokensToProcess.entries()) {
                ...
                if (walletsWithToken.length > 1) {
                    try {
                        ...
                        for (const { wallet, balance } of walletsWithToken) {
                            ...
                            if (balance >= 1) { // Minimum token amount threshold
                                sourceKeypairs.push((0, wallet_1.walletDataToKeypair)(wallet));
                        ...
                        if (sourceKeypairs.length > 0) {
                            ...
                            const bundleResult = await (0, transaction_1.bundleTokenTransfersFromSubwallets)(connection, sourceKeypairs, destinationPublicKey, tokenMints, amounts);
                            if (bundleResult.success) {
                                ...
                                if (bundleResult.errors.length > 0) {
                                    console.log(chalk_1.default.yellow(`Some operations encountered errors:`));
                                    bundleResult.errors.forEach((err, i) => {
                                        console.log(chalk_1.default.yellow(`  ${i + 1}. ${err}`));
                                    });
                                }
                            }
                            else {
                            ...
                            await (0, transaction_1.sleep)(bundleDelay);
                        }
                    }
                    catch (bundleError) {
                        spinner.fail(`Bundle processing error: ${bundleError.message}`);
                        console.log(chalk_1.default.yellow('Falling back to individual token transfers...'));
                        ...
                        for (let i = 0; i < walletsWithToken.length; i++) {
                            ...
                            try {
                                ...
                                // Transfer token
                                await (0, transaction_1.transferSplToken)(connection, keypair, destinationPublicKey, tokenMint, balance);
                                successCount++;
                                tokenTransferCount++;
                    ...
                    for (let i = 0; i < walletsWithToken.length; i++) {
                        ...
                        try {
                            ...
                            const transaction = await (0, transaction_1.transferSplToken)(connection, keypair, destinationPublicKey, tokenMint, balance);
                            successCount++;
                            tokenTransferCount++;
                            tokenTransferErrors = 0; // Reset error counter on success
                            // Increase the dynamic delay after successful transfer
                ...
                if (tokensMoved < tokensToProcess.size) {
                    ...
                    await (0, transaction_1.sleep)(mintDelay);
                }
            }
        ...
        for (const wallet of wallets) {
            ...
            try {
                ...
                await (0, transaction_1.sleep)(150);
                ...
                console.error(chalk_1.default.red(`Error checking wallet ${wallet.publicKey}: ${error.message}`));
                await (0, transaction_1.sleep)(500); // Add delay after errors
            }
        }
        ...
        for (let i = 0; i < eligibleWallets.length; i += WALLETS_PER_BUNDLE) {
            ...
            if (i > 0) {
                ...
                await (0, transaction_1.sleep)(batchDelay);
            }
            try {
                ...
                const bundleId = await (0, transaction_1.sendBundleFromMultipleWallets)(connection, keypairs, destinationPublicKey, transferAmounts);
                ...
                await (0, transaction_1.sleep)(successDelay);
            }
            catch (batchError) {
                ...
                if (batchError.message.includes('ENOTFOUND')) {
                    console.error(chalk_1.default.yellow(`DNS resolution failure detected. Check internet connection or Jito endpoint status.`));
                }
                else if (batchError.message.includes('429')) {
                    console.error(chalk_1.default.yellow(`Rate limit exceeded. Consider reducing batch frequency.`));
                }
                else if (batchError.message.includes('400')) {
                    console.error(chalk_1.default.yellow(`Bad request error (400). Jito may be rejecting the bundle format or parameters.`));
                }
                ...
                for (let i = 0; i < walletBatch.length; i++) {
                    try {
                        ...
                        const signature = await (0, transaction_1.transferSol)(connection, walletInfo.keypair, destinationPublicKey, walletInfo.balance);
                        ...
                        await (0, transaction_1.sleep)(1000);
                    }
                    catch (fallbackError) {
                        console.error(chalk_1.default.red(`Fallback processing failed for wallet: ${fallbackError.message}`));
                        failureCount++;
                    }
                }
        ...
        if (sellTokens && tokensToProcess.size > 0) {
            ...
            if (!destinationWalletData) {
                ...
                const destinationKeypair = (0, wallet_1.walletDataToKeypair)(destinationWalletData);
                // Get token balances of destination wallet
                const destinationTokens = await (0, transaction_1.getAccountTokens)(connection, destinationPublicKey);
                if (destinationTokens.length > 0) {
                    ...
                    if (!process.env.PUMPFUN_API_KEY) {
                        ...
                        console.log(chalk_1.default.yellow('To sell tokens with PumpFun, add your API key to the .env file:'));
                        console.log(chalk_1.default.yellow('PUMPFUN_API_KEY=your_api_key_here'));
                        const { addApiKey } = await inquirer_1.default.prompt([
                            {
                                type: 'confirm',
                                name: 'addApiKey',
                        ...
                        if (addApiKey) {
                            ...
                            try {
                                ...
                                console.log(chalk_1.default.green('PumpFun API key saved to .env file'));
                            }
                            catch (error) {
                    ...
                    const { confirmSell } = await inquirer_1.default.prompt([
                        {
                            type: 'confirm',
                            name: 'confirmSell',
                        ...
                        try {
                            ...
                            for (const token of destinationTokens) {
                                try {
                                    ...
                                    await (0, transaction_1.sleep)(sellDelay);
                                }
                                catch (sellError) {
                                    spinner.fail(`Error selling token ${token.mint.substring(0, 8)}...: ${sellError.message}`);
                                    console.log(chalk_1.default.gray('Details:', sellError.stack || sellError));
                                }
                            }
                            if (soldCount > 0) {
                                ...
                                console.log(chalk_1.default.yellow('- Jupiter Swap: https://jup.ag'));
                                console.log(chalk_1.default.yellow('- Raydium: https://raydium.io/swap'));
                            }
                        }
                        catch (error) {
        ...
        console.log(chalk_1.default.green(`Total SOL collected: ${totalSolCollected.toFixed(6)} SOL`));
        console.log(chalk_1.default.green(`Token types processed: ${tokensToProcess.size}`));
        console.log(chalk_1.default.green(`Successful transfers: ${successCount}`));
        console.log(chalk_1.default.green(`Failed transfers: ${failureCount}`));
        console.log(chalk_1.default.green(`Skipped wallets: ${skippedWallets}`));
        console.log(chalk_1.default.green(`Destination wallet: ${destinationPublicKey.toString()}`));
        if (sellTokens) {
        ...
        console.error(chalk_1.default.red(`Error during dust collection: ${error.message}`));
    }
}
exports.dustCommand = dustCommand;
