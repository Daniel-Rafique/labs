...
exports.checkBalancesCommand = void 0;
 ...
 */
async function retryWithBackoff(fn, maxRetries = 5, initialDelay = 500, spinner) {
    let retries = 0;
    ...
    while (true) {
        try {
            ...
            if (spinner) {
                ...
                console.log(chalk_1.default.yellow(`Rate limited. Retrying after ${Math.round(delay)}ms delay...`));
                await (0, transaction_1.sleep)(delay);
            }
        }
    }
 ...
 */
async function adaptiveDelay(spinner, customMessage, forceHighDelay = false) {
    ...
    if (rateLimit.consecutiveErrors > 0 || forceHighDelay) {
        ...
        delay = Math.min(delay * Math.pow(2, errorFactor), rateLimit.maxDelay);
    }
...
async function checkBalancesCommand(options) {
    try {
        ...
        if (includeTokens && wallets.length > 20 && !batchSize && !skipMost) {
            ...
            const modeResponse = await inquirer_1.default.prompt([
                {
                    type: 'list',
            ...
            largeWalletMode = modeResponse.largeWalletMode;
            // Apply selected handling
            if (largeWalletMode === 'withBalance') {
                skipMost = true;
                console.log(chalk_1.default.cyan('Will only check token balances for wallets with SOL balance > 0'));
            }
            else if (largeWalletMode === 'sample') {
                skipMost = true;
                // We'll handle the sample mode selection later
                console.log(chalk_1.default.cyan('Will only check token balances for 5 random wallets as a sample'));
            }
            else if (largeWalletMode === 'safe') {
                batchSize = 1; // Use smallest batch size
                console.log(chalk_1.default.cyan('Using reduced batch size and extended delays to process all wallets'));
            }
            else {
                // All wallets
                console.log(chalk_1.default.cyan('Checking tokens for all wallets (this might take a while)'));
            }
        }
        // Check balances
        const spinner = (0, ora_1.default)('Checking balances...').start();
        ...
        if (includeTokens) {
            ...
            await (0, transaction_1.sleep)(1500);
        ...
        for (let pageStart = 0; pageStart < wallets.length && shouldContinue; pageStart += PAGE_SIZE) {
            ...
            if (pageStart > 0 && wallets.length > PAGE_SIZE) {
                ...
                const { continueProcessing } = await inquirer_1.default.prompt([
                    {
                        type: 'confirm',
                ...
                if (!continueProcessing) {
                    console.log(chalk_1.default.yellow('Processing stopped by user after completing a page of wallets.'));
            ...
            for (let i = 0; i < currentPage.length; i += BATCH_SIZE) {
                ...
                for (const wallet of batchWallets) {
                    try {
                        ...
                        const balance = await retryWithBackoff(() => connection.getBalance(new web3_js_1.PublicKey(wallet.publicKey)), 5, // Max retries
                        500, // Initial delay
                        spinner);
                        ...
                        if (shouldCheckTokens) {
                            ...
                            try {
                                ...
                                const tokenAccounts = await retryWithBackoff(() => connection.getTokenAccountsByOwner(new web3_js_1.PublicKey(wallet.publicKey), { programId: token_compat_1.TOKEN_PROGRAM_ID }), 5, 500, spinner);
                                ...
                                const token2022Accounts = await retryWithBackoff(() => connection.getTokenAccountsByOwner(new web3_js_1.PublicKey(wallet.publicKey), { programId: token_compat_1.TOKEN_2022_PROGRAM_ID }), 5, 500, spinner);
                                    ...
                                    for (let j = 0; j < allTokenAccounts.length; j += TOKEN_BATCH_SIZE) {
                                        ...
                                        for (const tokenAccount of tokenBatch) {
                                            try {
                                                ...
                                                const accountInfo = await retryWithBackoff(() => connection.getTokenAccountBalance(tokenAccount.pubkey), 5, 500, spinner);
                                                ...
                                                await (0, transaction_1.sleep)(300);
                                            }
                                            catch (err) {
                                                console.error(chalk_1.default.yellow(`Error getting token balance: ${err.message}`));
                                ...
                                console.error(chalk_1.default.yellow(`Error checking token balances for ${wallet.publicKey.substring(0, 8)}: ${err.message}`));
                                rateLimit.consecutiveErrors++;
                                rateLimit.totalErrors++;
                                // Add recovery delay after error
                            ...
                            await (0, transaction_1.sleep)(100); // Tiny delay for UI feedback
                        }
                        ...
                        console.error(chalk_1.default.red(`Failed to check balances for wallet ${wallet.publicKey.substring(0, 8)}: ${err.message}`));
                        rateLimit.consecutiveErrors++;
                        rateLimit.totalErrors++;
                ...
                if (i + BATCH_SIZE < currentPage.length || pageStart + pageEnd < wallets.length) {
                    ...
                    for (let remaining = totalBatchDelay; remaining > 0; remaining -= stepSize) {
                        ...
                        await (0, transaction_1.sleep)(stepSize);
                    }
                }
                ...
                if (rateLimit.totalErrors > 10 && rateLimit.totalErrors % 5 === 0) {
                    ...
                    await (0, transaction_1.sleep)(cooldownTime);
        ...
        // Display summary
        console.log('\n' + chalk_1.default.cyan('====== BALANCE SUMMARY ======'));
        console.log(chalk_1.default.green(`Total wallets: ${wallets.length}`));
        console.log(chalk_1.default.green(`Wallets processed: ${balances.length}`));
        console.log(chalk_1.default.green(`Total SOL: ${totalSOL.toFixed(6)} SOL`));
        if (includeTokens) {
            console.log(chalk_1.default.green(`Total token accounts: ${totalTokens}`));
            if (skipMost) {
                console.log(chalk_1.default.yellow(`Note: Token checks were skipped for some wallets per selected strategy`));
            }
        }
        ...
        if (nonEmptyWallets.length > 0) {
            console.log('\n' + chalk_1.default.cyan('====== NON-EMPTY WALLETS ======'));
            for (const wallet of nonEmptyWallets) {
                console.log(chalk_1.default.yellow(`\nWallet: ${wallet.publicKey}`));
                console.log(chalk_1.default.green(`SOL Balance: ${wallet.solBalance.toFixed(6)} SOL`));
                if (wallet.tokenBalances && wallet.tokenBalances.length > 0) {
                    console.log(chalk_1.default.magenta('Token Balances:'));
                    for (const token of wallet.tokenBalances) {
                        console.log(chalk_1.default.cyan(`  ${token.mint.substring(0, 8)}... : ${token.uiAmount}`));
                    }
                }
            }
        }
        ...
        console.error(chalk_1.default.red(`Error checking balances: ${error.message}`));
    }
}
exports.checkBalancesCommand = checkBalancesCommand;
